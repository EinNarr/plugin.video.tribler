...
class Download(object):
	...
	def downloads_all(self):
		from resources.lib.module.perform_request import request_downloads
	...
	def download_action(self, info_hash, name):
		...
		if   action == 0:
			...
			if download_stopped:
				from resources.lib.module.perform_request import resume_download
				resume_download(self.api_port, info_hash)
			else:
				from resources.lib.module.perform_request import stop_download
	...
	def start_download(self, info_hash, name, auto=False):
		...
		from resources.lib.module.perform_request import start_download
		...
		if status == 200:
			...
			from resources.lib.module.utilities import show_warning
			show_warning(name, 'Download already exists')
		else:
			from resources.lib.module.utilities import show_error
